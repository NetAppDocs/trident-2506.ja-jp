...
.タスク概要
* アップグレード後に発見されたバグ、依存関係の問題、またはアップグレードの失敗や不完全なアップグレードの修正が必要な場合は、 Tridentをアンインストールし、そのバージョンの特定の手順に従って以前のバージョンを再インストールする必要があります。link:../earlier-versions.html["version"] 。これは、以前のバージョンにダウングレードする場合に推奨される唯一の方法です。
