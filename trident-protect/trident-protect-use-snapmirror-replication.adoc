...
NOTE: データ保護操作中にファイルシステムをフリーズおよびアンフリーズするようにTrident protect を構成できます。link:trident-protect-requirements.html#protecting-data-with-kubevirt-vms["Trident Protectを使用したファイルシステムのフリーズ設定の詳細"]。
...
実行フックの詳細については、以下を参照してください。link:../trident-protect/trident-protect-use-execution-hooks.html["Trident の保護実行フックを管理する"] 。
