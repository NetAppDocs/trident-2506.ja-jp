...
* サポートされているバージョンの Kubernetes を実行し、機能要件が有効になっている、サポートされている Kubernetes クラスターに対する完全な権限。レビューlink:requirements.html["要件"]詳細については。
...
適切なインストール方法を選択してください。また、以下の点についても検討する必要があります。link:kubernetes-deploy.html#move-between-installation-methods["方法間の移動"]決定を下す前に。



=== Trident演算子の使用

手動でデプロイする場合でも、Helm を使用する場合でも、 Tridentオペレーターはインストールを簡素化し、 Tridentリソースを動的に管理するのに最適な方法です。あなたはさらにlink:../trident-get-started/kubernetes-customize-deploy.html["Tridentオペレーターの展開をカスタマイズする"]の属性を使用して `TridentOrchestrator`カスタム リソース (CR)。
